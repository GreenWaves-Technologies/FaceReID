/*
  ...
  ** Modified for easy interrup pin assignement on method begin(datapin,irq_pin). Cuningan <cuninganreset@gmail.com> **
  ...
  Copyright (c) 2019 Xperience.AI
  Ported to Pulp OS for GAP8 platform (Gapuino) by Alexander Smorkalov
...
void ps2interrupt(void* arg)
{
    ...
    val = rt_gpio_get_pin_value(0, DataPin);
    now_ms = rt_time_get_us()/1000;
    if (abs(now_ms-prev_ms) > 250)
    {
        bitcount = 0;
        incoming = 0;
    }
...
const L2_MEM PS2Keymap_t PS2Keymap_US = {
  // without shift
...
static unsigned char get_iso8859_code(void)
{
...
void kb_begin(uint8_t data_pin, uint8_t irq_pin)
{
    DataPin = data_pin;
    keymap = &PS2Keymap_US;
    head = 0;
    ...
    rt_gpio_init(0, irq_pin);
    rt_gpio_set_dir(0, 1<<irq_pin, RT_GPIO_IS_IN);
    rt_gpio_set_pin_value(0, irq_pin, 1);

    rt_gpio_init(0, data_pin);
    rt_gpio_set_dir(0, 1<<data_pin, RT_GPIO_IS_IN);
    rt_gpio_set_pin_value(0, data_pin, 1);

    rt_gpio_set_sensitivity(0, irq_pin, RT_GPIO_SENSITIVITY_FALL);
    rt_event_t* int_event = rt_event_irq_get(ps2interrupt, NULL);
    if(!int_event)
    {
        PRINTF("ERROR: int_event not created\n");
    }
    rt_gpio_set_event(0, irq_pin, int_event);
}
