...
def compute_average_flops_cost(self):
    ...
    flops_sum = 0
    flops_by_type = defaultdict(int)
    for module in self.modules():
        if is_supported_instance(module):
            flops_sum += module.__flops__
            flops_by_type[module.__class__.__name__] += module.__flops__
    return flops_sum / batches_count, dict(flops_by_type)
...
def conv_flops_counter_hook(conv_module, input, output):
    ...
    conv_per_position_flops = kernel_height * kernel_width * filters_per_channel * out_channels
...
def count_flops(model, input_height, input_width, grayscale):
    with torch.cuda.device(0):
        num_channels = 3
        if grayscale:
            num_channels = 1
        batch = torch.FloatTensor(1, num_channels, input_height, input_width)
        model = add_flops_counting_methods(model)
        model.eval().start_flops_count()
        out = model(batch)

        total_flops, flops_by_type = model.compute_average_flops_cost()
        print('Output shape: {}'.format(list(out.shape)))
        print('Flops:  {}'.format(flops_to_string(total_flops)))
        for layer_type in flops_by_type:
            print('Flops for {}:  {}'.format(layer_type, flops_to_string(flops_by_type[layer_type])))
