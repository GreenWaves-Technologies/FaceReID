...
def absorb_bn(module, bn_module):
    ...
    bn_module.register_buffer('running_mean', torch.zeros(bn_module.running_mean.data.size(), dtype=torch.float32))
    bn_module.register_buffer('running_var', torch.ones(bn_module.running_var.data.size(), dtype=torch.float32))
...
def search_absorbed_bn(model):
    prev = None
    for m in model.children():
        if is_bn(m) and is_absorbing(prev):
            absorb_bn(prev, m)
        search_absorbed_bn(m)
        prev = m
