...
model_urls = {
    # top1 = 68.964
    'imagenet': 'https://raw.githubusercontent.com/billhhh/MnasNet-pytorch-pretrained/master/pretrained/top1_68.964%_model_best.pth.tar'
}
...
class MnasNet(nn.Module):
    def __init__(self, num_classes, loss, input_size=224, width_mult=1.):
        ...
        for expand_coeff, channels, num_blocks, stride, kernel in self.inverted_residual_setting:
            output_channel = int(channels * width_mult)
            for i in range(num_blocks):
                if i == 0:
                    self.features.append(InvertedResidual(input_channel, output_channel, stride, expand_coeff, kernel))
                else:
                    self.features.append(InvertedResidual(input_channel, output_channel, 1, expand_coeff, kernel))
                input_channel = output_channel
        self.features = nn.Sequential(*self.features)

        # building last several layers
        self.conv = Conv_1x1(input_channel, self.last_channel)
